...
public class Environment {
    ...
    public void onConfigurationChanged(Configuration newConfig, Context context) {
        ...
        if (mConfig.orientation != newConfig.orientation
            || mConfig.smallestScreenWidthDp != newConfig.smallestScreenWidthDp
            || mConfig.screenWidthDp != newConfig.screenWidthDp
            || mConfig.screenHeightDp != newConfig.screenHeightDp
            || mConfig.densityDpi != newConfig.densityDpi) {
        // MStar Android Patch End
            WindowManager wm = (WindowManager) context
                    .getSystemService(Context.WINDOW_SERVICE);
            Display d = wm.getDefaultDisplay();
            mScreenWidth = d.getWidth(); //*3/4-65;
            mScreenHeight = d.getHeight()*3/5;

            int scale;
            if (mScreenHeight > mScreenWidth) {
                mKeyHeight = (int) (mScreenHeight * KEY_HEIGHT_RATIO_PORTRAIT);
                mCandidatesAreaHeight = (int) (mScreenHeight * 2 * CANDIDATES_AREA_HEIGHT_RATIO_PORTRAIT);
                scale = mScreenWidth;
            } else {
