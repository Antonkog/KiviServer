...
public class SoftKeyboardView extends View {
    ...
    public SoftKeyboardView(Context context, AttributeSet attrs) {
        ...
        mActiveCellDrawable = r.getDrawable(R.drawable.btn_key_pressed);
    ...
    public boolean setSoftKeyboard(SoftKeyboard softSkb) {
        ...
        // First key to get selected when keyboard has changed
        Pair<Integer, Integer> keys = KeySelector.INSTANCE.getKeysToSelect();
        if (keys != null) {
            if (mSoftKeyboard.getKey(keys.first, keys.second) != null)
                mSoftKeyboard.setOneKeySelected(keys.first, keys.second);
            else mSoftKeyboard.setOneKeySelected(3, 4);
        } else {
            if (mSoftKeyboard.getKey(3, 5) != null)
                mSoftKeyboard.setOneKeySelected(3, 5);
            else mSoftKeyboard.setOneKeySelected(3, 4);
        }
...
//        mVibrator.vibrate(mVibratePattern, -1);
    ...
    private void drawSoftKey(Canvas canvas, SoftKey softKey, int keyXMargin,
        ...
        if (null != keyIcon) {
            ...
            int fontHeight = mFmi.bottom - mFmi.top - 16;
