...
public class SoftKeyboard {
    ...
    /**************************************************************
     * Set the specific one Softkey selected and others unselected.
     * @author mstar
     * @param mSoftKey
     ...
     *                 		set failure.
     ***************************************************************/
    public boolean setOneKeySelected(SoftKey mSoftKey) {
        if (mKeyRows.size() <= 0) {
            return false;
        }

        Log.d(TAG, "row:" + mKeyRows.size());
        for (int i = 0; i < mKeyRows.size(); i++) {
            Log.d(TAG, "row:" + i + "  index:" + mKeyRows.get(i).mSoftKeys.size());
            for (int j = 0; j < mKeyRows.get(i).mSoftKeys.size(); j++) {
                if (mKeyRows.get(i).mSoftKeys.get(j).mKeyCode == mSoftKey.mKeyCode) {
                    mKeyRows.get(i).mSoftKeys.get(j).setKeySelected(true);
                } else {
                    mKeyRows.get(i).mSoftKeys.get(j).setKeySelected(false);
                }
            }
        }
        return true;
     ...
     *                 		set failure.
     ***************************************************************/
    public boolean setOneKeySelected(int mRow, int mIndex) {
        if (mRow < 0 || mRow > mKeyRows.size() - 1) {
            return false;
        }

        if (mIndex < 0 || mIndex > mKeyRows.get(mRow).mSoftKeys.size() - 1) {
            return false;
        }
        ...
        mKeyRows.get(mRow).mSoftKeys.get(mIndex).setKeySelected(true);
        return true;
    }
    ...
    public void setSkbCoreSize(int skbCoreWidth, int skbCoreHeight) {
        Log.d("AAAAAAAAAAAAa", "skbCoreWidth:" + skbCoreWidth + "/" + "skbCoreHeight:" + skbCoreHeight);
        if (null == mKeyRows
    ...
    public SoftKey getKey(int row, int location) {
        if (null != mKeyRows && row >= 0 && mKeyRows.size() > row) {
            List<SoftKey> softKeys = mKeyRows.get(row).mSoftKeys;
            if ((softKeys.size() > location) && (location >= 0)) {
    ...
    public int getmapToKey(int x, int y, int row) {
        if (null == mKeyRows) {
            return -1;
        }
        // If the position is inside the rectangle of a certain key, return that
        ...
        KeyRow keyRow = mKeyRows.get(row);
        if (KeyRow.ALWAYS_SHOW_ROW_ID != keyRow.mRowId
                && keyRow.mRowId != mEnabledRowId) return -1;
        if (keyRow.mTop > y && keyRow.mBottom <= y) return -1;
        ...
        for (int row = 0; row < rowNum; row++) {
            ...
            for (int keyPos = 0; keyPos < keyNum; keyPos++) {
                ...
                if (needUpdateQwerty) {
                    ...
                    if (sKey.mKeyCode >= RussianKeyCodes.RUSSIAN_F
                            && sKey.mKeyCode <= RussianKeyCodes.RUSSIAN_Z) {
                        sKey.changeCase(isQwertyUpperCase);
                    }

                    if (sKey.mKeyCode >= RussianKeyCodes.RUSSIAN_F
                            && sKey.mKeyCode <= UkrainianKeyCodes.UKRAINIAN_G) {
                        sKey.changeCase(isQwertyUpperCase);
                    }
                }
            }
