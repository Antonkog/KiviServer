...
public class SkbContainer extends RelativeLayout implements OnTouchListener {
    ...
    public SkbContainer(Context context, AttributeSet attrs) {
        ...
        mPopupSkb = new PopupWindow(context);
        mPopupSkb.setBackgroundDrawable(null);
        mPopupSkb.setBackgroundDrawable(getResources().getDrawable(R.drawable.selected_pannel));
...
//                    & (EditorInfo.IME_MASK_ACTION | EditorInfo.IME_FLAG_NO_ENTER_ACTION);
    ...
    public void actionForEnterDown() {
        ...
        mSkv = mMajorView;
        if (null != mSkv) {
            mSoftKeyDown = mSkv.getCurrentSelectKey();
    ...
    /**************************************************
     * Get curent selected softkey.
     * @return softkey
     *************************************************/
    public SoftKey getCurrentSelectedKey() {
        return mMajorView.getCurrentSelectKey();
    }

    public boolean getInputViewFocusable() {
        return mMajorView.getSkbViewFocusable();
    }

    public void setInputViewFocusable(boolean focusable) {
        mMajorView.setSkbViewFocusable(focusable);
    }

    public void setCandViewOnShow(boolean onShow) {
        mMajorView.setCandViewOnShow(onShow);
    }

    public void toggleCandidateMode(boolean candidatesShowing) {
        if (null == mMajorView || mLastCandidatesShowing == candidatesShowing) return;
    ...
    private void updateSkbLayout() {
        ...
        if (mSkbLayout == R.xml.skb_qwerty_en) {
            majorSkb = skbPool.getSoftKeyboard(R.xml.skb_qwerty_en,
                    R.xml.skb_qwerty_en, screenWidth, skbHeight, mContext);

        } else if (mSkbLayout == R.xml.skb_sym1) {
            majorSkb = skbPool.getSoftKeyboard(R.xml.skb_sym1, R.xml.skb_sym1,
                    screenWidth, skbHeight, mContext);

        } else if (mSkbLayout == R.xml.skb_sym2) {
            majorSkb = skbPool.getSoftKeyboard(R.xml.skb_sym2, R.xml.skb_sym2,
                    screenWidth, skbHeight, mContext);

        } else if (mSkbLayout == R.xml.skb_smiley) {
            majorSkb = skbPool.getSoftKeyboard(R.xml.skb_smiley,
                    R.xml.skb_smiley, screenWidth, skbHeight, mContext);

        } else if (mSkbLayout == R.xml.skb_phone) {
            majorSkb = skbPool.getSoftKeyboard(R.xml.skb_phone,
                    R.xml.skb_phone, screenWidth, skbHeight, mContext);

        } else if (mSkbLayout == R.xml.skb_qwerty_ru) {
            majorSkb = skbPool.getSoftKeyboard(R.xml.skb_qwerty_ru,
                    R.xml.skb_qwerty_ru, screenWidth, skbHeight, mContext);
        } else if (mSkbLayout == R.xml.skb_qwerty_ua) {
            majorSkb = skbPool.getSoftKeyboard(R.xml.skb_qwerty_ua,
                    R.xml.skb_qwerty_ua, screenWidth, skbHeight, mContext);
        } else {
    ...
    private void responseKeyEvent(SoftKey sKey) {
        ...
        if (66 == sKey.mKeyCode) {
            ((PinyinIME) mService).sendKeyChar('\n');
            return;
        }
